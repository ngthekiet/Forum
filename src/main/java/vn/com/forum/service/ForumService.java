package vn.com.forum.service;

import org.springframework.stereotype.Service;
import vn.com.forum.model.Category;
...
public class ForumService {
    ...
    public boolean newTopic(Topic topic, User user) {
        try {
            Topic newTopic = new Topic();
            newTopic.setId(getSizeTopic() + 1);
            newTopic.setTitle(topic.getTitle());
            newTopic.setContent(topic.getContent());
            newTopic.setCreator(user);
            newTopic.setCreatedTime(Calendar.getInstance());
            newTopic.setCategory(new Category("Học hành"));
            topics.put(getSizeTopic() + 1, newTopic);
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    public boolean addMessage(Message message, int id, User user) {
        try {
            message.setCreatedTime(Calendar.getInstance());
